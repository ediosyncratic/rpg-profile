...
<table>
  ...
  <tbody>
    {section name="i" loop="$characters"}
      <tr><td><a href="view.php?id={$characters[i].id}">{$characters[i].cname}</a></td><td>{$characters[i].lastedited}</td><td>{$characters[i].template}</td></tr>
    {sectionelse}
...
{if $prevpage}
  <a href="search.php?type={$type}&amp;cname={$cname}&amp;page={$prevpage}{$np}">&lt; Previous</a>
  {if $nextpage}|{/if}
{/if}
{if $nextpage}
  <a href="search.php?type={$type}&amp;cname={$cname}&amp;page={$nextpage}{$np}">Next &gt;</a>
{/if}
<br><br>
<h1>Search</h1>
